...
import time
import logging
from models import AddInput, AddOutput, SqrtInput, SqrtOutput, StringsToIntsInput, StringsToIntsOutput, ExpSumInput, ExpSumOutput, ProductChunkTyped, ProductMetadata, ProductResponse
from PIL import Image as PILImage
...
@mcp.tool()
def ask_user_for_clarification_feedback(original_user_query: str, query_from_llm: str) -> str:
    """
    Prompt the user for clarification or feedback on their original query.

    Args:
        original_user_query (str): The initial query entered by the user.
        query_from_llm (str): A follow-up or clarification question generated by the LLM. 
            For example, if the original query is "I want to buy a Nike T-shirt", the LLM might ask 
            "Is it for casual wear or sports wear?" — in this case, query_from_llm would be that question.

    Returns:
        str: A modified query that combines the original query and the user's clarification feedback.
    """
    feedback_query = input(f"Clarification needed: {query_from_llm}\nYour response: ")
    return original_user_query + " " + feedback_query
